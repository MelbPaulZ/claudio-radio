...
/**
 * 缓存 key = sha256("volc::<voice>::<text>") 前 16 位
 * 导出以便单元测试。换 voice 会自然改变 key，不会命中旧缓存。
 */
export function buildCacheKey(voice, text) {
  return crypto.createHash('sha256')
    .update(`volc::${voice}::${text}`)
    .digest('hex')
    .slice(0, 16);
}
 ...
 */
export function buildVolcBody({ appid, token, cluster, voice, uid, reqid, text }) {
  return {
...
export async function synth(text) {
  ...
  const hash = buildCacheKey(VOLC_VOICE, text);
